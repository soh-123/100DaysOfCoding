...
if start == "y":
    print(
        "Your ship has just landed on the mystery island, there are two different ways\n One of your crew suggested going left and another suggested the right way.\n "
    )
    choice1 = input(
        "Which way do you take? Write Right or Left to continue ..\n"
    ).lower()
    if choice1 == "left":
        print(
            "That way leading to a beatiful lake, if you swim you will cross the lake faster but since it's a mysterious island you could keep going around the lake to get to the other side\n"
        )
        choice2 = input(
            "Which way will you take? write Swim or Wait to continue .. \n"
        ).lower()
        if choice2 == "wait":
            print(
                "Smart choice, now you walk longer but more safely.\n You have found the treasure, but before you approach a wise man comes to tell you that only one has the treasure and the rest will cause your death\n"
            )
            choice3 = input(
                "Which box will you open? Write red, blue, or yellow to continue ..\n"
            ).lower()
            if choice3 == "yellow":
